...
class TimeLogManager:
    ...
    def create_styles(self):
        ...
        style.configure('Total.Treeview', background='#e6f3ff', 
    ...
    def create_gui(self):
        ...
        self.notebook.add(self.project_report_tab, text="Project Tasks Report")
    ...
    def create_time_log_widgets(self, parent):
        ...
        fields = [
            ...
            ("Employ:", "employ_combobox", ttk.Combobox(input_frame, state='readonly', width=40)),
        ...
        ttk.Button(buttons_frame, text="Add Entry", command=self.add_time_log, 
                 style='Accent.TButton').grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        ttk.Button(buttons_frame, text="Update Entry", command=self.update_time_log, 
                 style='Accent.TButton').grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ttk.Button(buttons_frame, text="Delete Entry", command=self.delete_time_log, 
        ...
        columns = ("Log ID", "Date", "Client", "Project", "Task", "Employ", "Hours", "Notes")
        ...
        for col in columns:
            ...
            self.time_log_tree.column(col, anchor='center', width=140)
        self.time_log_tree.column("Hours", width=80)
    ...
    def create_view_by_date_widgets(self, parent):
        ...
        self.view_date_button = ttk.Button(control_frame, text="View", 
                                         command=self.view_logs_by_date, style='Accent.TButton')
        ...
        for col in columns:
            ...
            self.view_date_tree.column(col, anchor='center', width=140)
        self.view_date_tree.column("Hours", width=80)
    ...
    def create_project_report_widgets(self, parent):
        ...
        report_frame = ttk.LabelFrame(parent, text="Task Schedule and Hours", padding=10)
        ...
        for col in columns:
            ...
            self.report_tree.column(col, anchor='center', width=column_widths[col])
    ...
    def populate_dropdowns(self):
        try:
            ...
            if client_names:
                ...
                self.report_client_combobox.set('')
            ...
            self.show_status_message(f"Error fetching data: {err}", error=True)

    def on_client_selected(self, event=None):
        client_name_with_id = self.client_combobox.get()
        if not client_name_with_id:
    ...
    def on_report_client_selected(self, event=None):
        ...
        if not client_name_with_id:
            self.report_project_combobox['values'] = []
            self.report_project_combobox.set('')
            return
        ...
        try:
            ...
            projects = self.cursor.fetchall()
            project_names = [f"{proj[1]} ({proj[0]})" for proj in projects]
            self.report_project_combobox['values'] = project_names
            if project_names:
                self.report_project_combobox.current(0)
            else:
                self.report_project_combobox.set('')
        except mysql.connector.Error as err:
            self.show_status_message(f"Error fetching projects: {err}", error=True)
    ...
    def populate_time_log_list(self):
        ...
        try:
            self.cursor.execute("""
                ...
                FROM time_log ORDER BY log_date DESC, log_id DESC
            """)
            ...
            for log in logs:
                ...
                self.time_log_tree.insert("", tk.END, values=(
                    ...
                    task_name,
                    employ_name,
                    f"{log[6]:.2f}" if log[6] is not None else "",
                    log[7] if log[7] else ""
                ))
        except mysql.connector.Error as err:
    ...
    def view_logs_by_date(self):
        selected_date_dt = self.filter_date_entry.get_date()
        selected_date = selected_date_dt.strftime("%Y-%m-%d")
        self.view_date_tree.delete(*self.view_date_tree.get_children())

        try:
            # Fetch logs and calculate total hours in one transaction
            self.cursor.execute("""
                SELECT log_id, log_date, client_id, project_no, task_id, employ_id, hours, notes
                FROM time_log WHERE log_date=%s
                ORDER BY log_date DESC, log_id DESC
            """, (selected_date,))
            logs = self.cursor.fetchall()

            self.cursor.execute("""
                SELECT COALESCE(SUM(hours), 0.0) FROM time_log WHERE log_date=%s
            """, (selected_date,))
            total_hours = self.cursor.fetchone()[0]
            self.total_hours_label.config(text=f"Total Hours: {total_hours:.2f}")

            if not logs:
                self.show_status_message(f"No time logs found for {selected_date}")
            else:
                self.show_status_message(f"Showing time logs for {selected_date}")
            ...
            self.show_status_message(f"Error loading time logs by date: {err}", error=True)
    ...
    def generate_project_report(self):
        ...
        try:
            ...
            self.cursor.execute("""
                SELECT task_id, task_name FROM task 
                WHERE project_no=%s ORDER BY task_id
            """, (project_no,))
            tasks = self.cursor.fetchall()
            ...
            # Get task data in bulk for efficiency
            self.cursor.execute(f"""
                ...
                GROUP BY task_id
            """, tuple(task_ids))
            task_data = {row[0]: row[1:] for row in self.cursor.fetchall()}
            ...
            for task in tasks:
                ...
                task_info = task_data.get(task_id, (None, None, 0.0))
                start_date = task_info[0].strftime("%Y-%m-%d") if task_info[0] else "N/A"
                end_date = task_info[1].strftime("%Y-%m-%d") if task_info[1] else "N/A"
                total_hours = float(task_info[2]) if task_info[2] else 0.0
                total_project_hours += total_hours
                
                employees = task_employees.get(task_id, "N/A")
            ...
            self.show_status_message("Project task report generated successfully")
            
        except mysql.connector.Error as err:
            self.show_status_message(f"Error generating report: {err}", error=True)

    def add_time_log(self):
        log_date_dt = self.date_entry.get_date()
        log_date = log_date_dt.strftime("%Y-%m-%d")
        ...
        client_id = self._extract_id_from_combobox(client_name_with_id)
        project_no = self._extract_id_from_combobox(project_name_with_no)
        task_id_str = self._extract_id_from_combobox(task_name_with_id)
        employ_id = self._extract_id_from_combobox(employ_name_with_id)
        
        if None in (client_id, project_no, task_id_str, employ_id):
            self.show_status_message("Invalid selection for Client, Project, Task, or Employ.", error=True)
            return

        try:
            task_id = int(task_id_str)
        except ValueError:
            self.show_status_message("Invalid Task ID format", error=True)
            return
        ...
        try:
            ...
            self.conn.commit()
            self.show_status_message("Time log entry added")
            self.populate_time_log_list()
            self.clear_inputs()
        except mysql.connector.Error as err:
            if err.errno == 1062:
                self.show_status_message("Duplicate entry for Log ID. Change date or task.", error=True)
            else:
    ...
    def update_time_log(self):
        ...
        if not selected:
            self.show_status_message("Select a time log entry to update", error=True)
            return
        ...
        try:
            if new_log_id != old_log_id:
                self.cursor.execute("""
                    SELECT log_id FROM time_log 
                    WHERE log_id=%s AND log_id <> %s
                """, (new_log_id, old_log_id))
                if self.cursor.fetchone():
                    self.show_status_message("Another entry with this Log ID already exists.", error=True)
                    return
            ...
            self.cursor.execute(
                ...
                WHERE log_id=%s""",
                (new_log_id, log_date, client_id, project_no, task_id, 
                 employ_id, hours_val, notes if notes else None, old_log_id)
            )
            self.conn.commit()
            self.show_status_message("Time log entry updated")
            self.populate_time_log_list()
            self.clear_inputs()
        except mysql.connector.Error as err:
    ...
    def delete_time_log(self):
        selected = self.time_log_tree.selection()
        if not selected:
            self.show_status_message("Select a time log entry to delete", error=True)
            return
        log_id = self.time_log_tree.item(selected[0])['values'][0]
        
        if messagebox.askyesno("Confirm Delete", f"Delete log ID '{log_id}'?"):
            try:
                self.cursor.execute("DELETE FROM time_log WHERE log_id=%s", (log_id,))
                self.conn.commit()
                self.show_status_message("Time log entry deleted")
                self.populate_time_log_list()
                self.clear_inputs()
            except mysql.connector.Error as err:
    ...
    def on_time_log_select(self, event):
        ...
        self.populate_project_dropdown(client_id)
        ...
        self.populate_task_dropdown(project_no)
    ...
    def set_combobox_value(self, combobox, display_name):
        for val in combobox['values']:
            if val.startswith(display_name + ' ('):
                combobox.set(val)
                return
        combobox.set('')
    ...
    def _extract_id_from_combobox(self, combobox_value):
        if not combobox_value:
            return None
        start = combobox_value.rfind('(')
        end = combobox_value.rfind(')')
        if start != -1 and end != -1 and end > start:
            return combobox_value[start + 1:end].strip()
        return None

    def _generate_log_id(self, log_date, task_id):
        date_str = log_date.replace('-', '')  # YYYYMMDD
        return f"{date_str}{task_id}"
    ...
    def fetch_name(self, table, id_value, id_col, name_col):
        ...
        try:
            ...
            return str(row[0]) if row else str(id_value)
    ...
    def populate_project_dropdown(self, client_id):
        if not client_id:
            self.project_combobox['values'] = []
            self.project_combobox.set('')
            self.task_combobox['values'] = []
    ...
    def populate_task_dropdown(self, project_no):
        if not project_no:
            self.task_combobox['values'] = []
            self.task_combobox.set('')
            return
